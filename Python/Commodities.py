...
class Commodity():
    ...
    def setgen(self,rng:Union[np.random.Generator,tf.random.Generator]):
        self.rng=rng

class Schwartz2Factor(Commodity):
    def brownianMotionNP(T:float,N:int,M:int,
                    d:Optional[int]=1,
    ...
    # @tf.function(jit_compile=True) #slower
    @tf.function
    ...
    # @staticmethod
    def schwartz2factorNP(delta0:float,S0:float,r:float,sigma1:float,sigma2:float,kappa:float,alpha:float,l:float,t:np.ndarray,W:np.ndarray,
                        dtype:Optional[DTypeLike]=np.float32)\
        ->np.ndarray:
        N=t.size
        dt=t[-1]/(N-1)
    ...
    def __init__(self, 
        ...
        if type(t)==np.ndarray:
            self._d=2
            self._brownianMotion=Schwartz2Factor.brownianMotionNP
            self._schwartz2Factor=Schwartz2Factor.schwartz2factorNP
